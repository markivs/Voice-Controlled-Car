...
/*    PREPROGRAMMED PATH     */
/*---------------------------*/
int run_times[NUM_COMMANDS] = {7000, 2000, 2500, 2500}; // length of commands roughly in ms
...
float delta_reference(int n) {
  ...
  if (drive_mode == DRIVE_RIGHT) { // Return a NEGATIVE expression
    return (TURN_RADIUS/2-CAR_WIDTH/2) * n * v_star/(TURN_RADIUS/2*5) - (TURN_RADIUS/2+CAR_WIDTH/2) * n * v_star/(TURN_RADIUS/2*5);
